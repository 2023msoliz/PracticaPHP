...
<body>
    ...
    <ul>
    <?php
    // Obtiene un numero aleatorio de 0-5
    $taules = array();
    for($i = 0; $i < 10; $i++) {
        array_push($taules, rand(0, 5));
    }
    // Muestra el estado de las mesas en una lista
    for($i = 0; $i < count($taules); $i++) {
        ...
        if($taules[$i] == 0){
            ...
            echo "La taula $i té " . $taules[$i] . " comensals";
        }
        echo "</li>";
    }
    ?>
   ...
   // Inicializamos todas las habitaciones con 0 clientes
   for ($planta = 0; $planta < 5; $planta++) {
        for ($habitacion = 0; $habitacion < 10; $habitacion++) {
            $hotel[$planta][$habitacion] = 0;
        }
    }
    ?>
</body>
